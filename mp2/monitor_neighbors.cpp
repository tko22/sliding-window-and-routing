...
void writeSendLog(char *theLogFile, int16_t dest, int16_t nexthop,
                  char *message) {
...
void writeForwardLog(char *theLogFile, int16_t dest, int16_t nexthop,
                     char *message) {
    char logLine[LOG_LINE_LENGTH];
    memset(logLine, '\0', LOG_LINE_LENGTH);
...
void writeUnreachableLog(char *theLogFile, int16_t dest) {
    char logLine[LOG_LINE_LENGTH];
    memset(logLine, '\0', LOG_LINE_LENGTH);
...
void updateCost(int16_t nodeID, int32_t cost) {
...
void updateConnection(int16_t nodeID, bool connect) {
    if (nodeID >= 0 && nodeID < 256) {
...
void hackyBroadcast(const char *buf, int length) {
    int i;
    for (i = 0; i < 256; i++)
        if (i != globalMyID)  //(although with a real broadcast you would also
                              // get the packet yourself)
...
void listenForNeighbors() {
    ...
    while (1) {
        ...
        if (strstr(fromAddr, "10.1.1.")) {
            ...
            // TODO: this node can consider heardFrom to be directly connected
            // to it; do any such logic now.
        ...
        // order 4 byte signed>
            ...
            // TODO record the cost change (remember, the link might currently
            // be down! in that case, this is the new cost you should treat it
            // as having once it comes back up.)

            updateCost(ntohs(recvBuf[4]), ntohl(recvBuf[6]));
        }

        // TODO now check for the various types of packets you use in your own
        // protocol else if(!strncmp(recvBuf, "your other message types", ))
        // ...
    }
